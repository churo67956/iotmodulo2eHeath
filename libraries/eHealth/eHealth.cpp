// eHealth Library Simulation version V20
// Added to library V10 version a function for weight acquisition simulation
// getWeight() provides a random weight data in the ( WEIGHT_RANDOM_MIN, WEIGHT_RANDOM_MAX ) range
...
//***************************************************************
	...
	//!******************************************************************************
	//!		Name:	getWeight()														*
	//!		Description: Returns the weight.										*
	//!		Param : void															*
	//!		Returns: float with the weight value.									*
	//!		Example: float weight = eHealth.getWeight();							*
	//!******************************************************************************

	float eHealthClass::getWeight(void)
	{	
		//Local variables
		float Weight; //Weight 
		
		Weight = ( float )random( WEIGHT_RANDOM_MIN, WEIGHT_RANDOM_MAX + 1 );
		Weight = Weight / 100;
	...
	{	
		...
		Temperature = ( float )random( TEMPERATURE_RANDOM_MIN, TEMPERATURE_RANDOM_MAX + 1 );
	...
	int eHealthClass::getOxygenSaturation(void)
	{
		...
		SPO2 = ( int )random( SPO2_RANDOM_MIN, SPO2_RANDOM_MAX + 1 );
		return SPO2;
	}
	...
	{
		BPM = ( int )random( BPM_RANDOM_MIN, BPM_RANDOM_MAX + 1 );
		return BPM;
	}
	...
	void eHealthClass::readPulsioximeter(void)
	{
		...
		for (int i = 0; i<41 ; i++) { // read all the led's of the module
			...
			SPO2 = 10 * digito[26] + digito[20];
			BPM  = 100 * digito[13] + 10 * digito[7] + digito[0];
